class DrugsController < ApplicationController
  ...
  # GET /drugs/1.json
  def show
    @reviews = Review.where(drug_id: @drug.id).order("created_at DESC")
  end
  ...
  def create
    ...
    disease = Disease.find(drug_params[:disease_id])
    @drug.build_disease(:id => disease.id)
  ...
  private
    ...
    def drug_params
      params.require(:drug).permit(:name, :uses, :sideeffects, :precautions, :interactions, :overdose, :disease_id, :image)
    end
end
